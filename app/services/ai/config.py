...
class AIModel(Enum):
    ...
    GEMINI_3_FLASH = (
        ...
        AIProvider.GEMINI,
    )  # Latest Flash with Pro-grade reasoning
    GEMINI_3_PRO = (
        "gemini-3-pro-preview",
        AIProvider.GEMINI,
    )  # Latest Pro for agentic workflows

    # Gemini 2.5 models (Stable)
    GEMINI_FLASH_25 = (
        "gemini-2.5-flash",
        AIProvider.GEMINI,
    )  # Stable version - fast & intelligent
    GEMINI_PRO_25 = ("gemini-2.5-pro", AIProvider.GEMINI)  # Best quality
    GEMINI_FLASH_LITE_25 = (
        "gemini-2.5-flash-lite",
        AIProvider.GEMINI,
    )  # High-throughput, cost-optimized

    # Gemini aliases (auto-update to latest) - RECOMMENDED
    GEMINI_FLASH_LATEST = (
        "gemini-flash-latest",
        AIProvider.GEMINI,
    )  # Recommended default - always latest Flash
    GEMINI_PRO_LATEST = (
        "gemini-pro-latest",
        AIProvider.GEMINI,
    )  # Alias: latest Pro version
    ...
    def from_string(cls, model_name: str) -> "AIModel":
        ...
        if result is None:
            logger.warning(
                f"Unknown model '{model_name}', using default GEMINI_FLASH_25"
            )
...
class AIConfig:
    ...
    def from_env(cls) -> "AIConfig":
        ...
        try:
            config.default_temperature = float(
                os.getenv("AI_DEFAULT_TEMPERATURE", "0.7")
            )
        except ValueError:
        ...
        # Log configuration
        logger.info(
            f"AI Config loaded: default={config.default_model.model_id}, "
